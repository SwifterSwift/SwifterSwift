...
final class UIViewExtensionsTests: XCTestCase { // swiftlint:disable:this type_body_length
    ...
    func testAddGradient() throws {
        ...
        if let sublayers = view0.layer.sublayers as? [CAGradientLayer] {
            ...
            let colors = try XCTUnwrap(sublayers[0].colors as? [CGColor])
            XCTAssertEqual(colors.count, 4)
            XCTAssertEqual(colors[0], UIColor.red.cgColor)
            XCTAssertEqual(colors[1], UIColor.orange.cgColor)
            XCTAssertEqual(colors[2], UIColor.green.cgColor)
            XCTAssertEqual(colors[3], UIColor.blue.cgColor)
