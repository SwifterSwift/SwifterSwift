...
public extension UIColor {
    ...
    /// Create new UIColor from RGB values.
    ...
    /// Create new UIColor from RGB values with transparency.
    ...
    /// Create new UIColor from hexadecimal value.
    public convenience init(netHex:Int) {
        self.init(red:(netHex >> 16) & 0xff, green:(netHex >> 8) & 0xff, blue:netHex & 0xff)
    }

    /// Create new UIColor from hexadicimal value with transparency.
    ...
    /// Return hexadecimal value string (read-only).
    ...
    /// Return a random color (read-only).
